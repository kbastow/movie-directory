...
The app reads this via `import.meta.env.VITE_TMDB_API_KEY`.
...
Deployment targets GitHub Pages at:
`https://kbastow.github.io/movie-directory`
...
- Keep API access isolated to `src/api/*`
- Add data-access behavior through hooks in `src/hooks/*`
- Keep route-level concerns in `src/pages/*` and reusable pieces in `src/components/*`
- Maintain type contracts in `src/types/*` when extending TMDB data usage
